...
T_INITIAL = 1e-7
#Factor de enfriamiento (alpha_cool)
COOLING_FACTOR = 0.99999 # Reducción del 0.001% por paso de temperatura.
# Longitud de la Cadena de Markov (L)
ITER_PER_TEMP = 1000000 # Un millón de iteraciones por nivel de T.
# Inicializar un contador auxiliar
...
try:
    while (time.time() - start_time) < TIME_LIMIT_SECONDS:
        ...
        if delta_score > 0:
            ...
            # --- LÓGICA DE ENFRIAMIENTO CONTROLADO ---
            # Se aplica el factor de enfriamiento solo después de L iteraciones
            temp_iteration_counter += 1

            if temp_iteration_counter >= ITER_PER_TEMP:
                # 1. Aplicar la reducción de temperatura
                current_temp *= COOLING_FACTOR
                
                T_FINAL = 1e-10 # Temperatura final de parada

                if current_temp < T_FINAL:
                    print(f"Temperatura final de {current_temp:.2e} alcanzada. Finalizando búsqueda.")
